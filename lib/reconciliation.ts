...
function classify(params: {
  ...
  if (sellerRecords.length > 1) {
    return {
      status: 'DUPLICATE_RECORD',
      reason: `This order ID appears ${sellerRecords.length} times in your sales report. Duplicate entries need review before reconciliation totals can be trusted.`,
    };
  }

  const expected = expectedAmountOf(seller);
